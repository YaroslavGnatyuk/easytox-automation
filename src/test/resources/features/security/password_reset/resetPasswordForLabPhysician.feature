Feature: Security framework test
  ...
  Scenario: Password Reset for a lab user
    ...
    When Enter Username as "SPhyTwo", valid email address and click "Send my Password" button.
    ...
    And Forgot Password   "SPhyTwo" <link>
    And See All Requests <Link>

    When Select Forgot Password   "SPhyTwo".
    ...
    Then "SPhyTwo password reset successfully" message should be displayed.

    When Login to the Easytox with login "SPhyTwo" and password "Welcome@123" credentials.
    Then "Change Password" screen with following fields should be displayed: Username
    And Old Password
    And New Password
    And Confirm Password

    When Verify username "SPhyTwo" populated in "Username" field.
    ...
    When Enter "SPhyTwo" and "Test@1234" as user credentials and login.
