package com.easytox.automation.steps.security.search.brigade;
...
public class SearchForBrigadePatientSteps {
    ...
    private int patientsNotFoundFromWestLab = 0;

    private Logger log = Logger.getLogger(SearchForBrigadePatientSteps.class);
    ...
    public void checkDisplayedPatients() {
        try {
            ...
            assertEquals("Patients from west lab should be in list"
                    , true, allPatients.containsAll(westPatients));
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
    ...
    public void checkPatientsList() {
        try {
            ...
            /*if (!allPatients.isEmpty()) {
                throw new PatientSearchException("There should be only patients from WestLab \n" +
    ...
    public void searchExistPatients() {
        for (String patient : westPatients) {
    ...
    public void resultSearch(){
        assertEquals("All user should be to found, so 'patientsNotFoundFromWestLab' should be to equal zero",
                true, patientsNotFoundFromWestLab == 0);
    }
