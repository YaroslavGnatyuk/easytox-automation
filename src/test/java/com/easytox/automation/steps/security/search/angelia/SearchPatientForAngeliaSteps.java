package com.easytox.automation.steps.security.search.angelia;
...
public class SearchPatientForAngeliaSteps {
    ...
    private int patientsNotFoundFromQuestAndWestLab = 0;

    private Logger log = Logger.getLogger(SearchPatientForAngeliaSteps.class);
    ...
    public void checkDisplayedPatients() {
        try {
            ...
            assertEquals("Patients from west and quest lab should be in list"
                    , true, allPatients.containsAll(questPatients) && allPatients.containsAll(westPatients));
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
    ...
    public void checkPatientsList() {
        try {
            ...
            allPatients.removeAll(westPatients);

            /*if (!allPatients.isEmpty()) {
                throw new PatientSearchException("There should be only patients from\n" +
                        "QuestLab and belongs Angelia in WestLab\n" +
    ...
    public void searchExistPatients() {
        searchPatients(questPatients);
        searchPatients(westPatients);
    }

    private void searchPatients(List<String> patients) {
        for (String patient : patients) {
            ...
            try {
                ...
                patientsNotFoundFromQuestAndWestLab++;
    ...
    public void resultSearchExistPatients(){
        assertEquals("All user should be to found, so 'patientsNotFoundFromQuestAndWestLab' should be to equal zero",
                true, patientsNotFoundFromQuestAndWestLab == 0);
    }
