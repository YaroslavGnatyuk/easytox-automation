package com.easytox.automation.steps.security.search.bradlemon;
...
public class SearchPatientForBradLemonSteps {
    ...
    private int patientsNotFoundFromWestAndZestLab = 0;

    private Logger log = Logger.getLogger(SearchPatientForBradLemonSteps.class);
    ...
    public void checkDisplayedPatients() {
        try {
            ...
            assertEquals("Patients from west and zest lab should be in list"
                    , true, allPatients.containsAll(zestPatients) && allPatients.containsAll(westPatients));
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
    ...
    public void resultSearch() {
        assertEquals("All user should be to found, so 'patientsNotFoundFromWestLab' should be to equal zero",
                true, patientsNotFoundFromWestAndZestLab == 0);
    }
    ...
    private void searchPatients(List<String> patients) {
        for (String patient : patients) {
            ...
            try {
                ...
                patientsNotFoundFromWestAndZestLab++;
